...
fn finish_onboarding(
    ...
    // On Windows, restore overlay window properties.
    #[cfg(target_os = "windows")]
    {
        let handle = app_handle.clone();
        let _ = app_handle.run_on_main_thread(move || {
...
/// Shows the onboarding window on macOS. Sizes the main window, centers it,
/// makes it visible, and emits `thuki://onboarding` so the frontend switches
/// to `OnboardingView`.
#[cfg(target_os = "macos")]
#[cfg_attr(coverage_nightly, coverage(off))]
fn show_onboarding_window(app_handle: &tauri::AppHandle, stage: onboarding::OnboardingStage) {
