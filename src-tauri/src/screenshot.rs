...
///
/// On macOS, this hides the window, runs `screencapture -i -x`, then re-shows.
/// On Windows, this delegates to the windows_screenshot module.
#[cfg(target_os = "macos")]
...
/// Non-macOS, non-Windows stub: full-screen capture is not available on unknown platforms.
#[cfg(not(any(target_os = "macos", target_os = "windows")))]
fn capture_full_screen_pixels() -> Result<(u32, u32, Vec<u8>), String> {
    Err("full-screen capture is only supported on macOS and Windows".to_string())
}

/// Tauri command: silently captures the full screen and returns the absolute
/// file path of the saved image.
///
/// On macOS, uses CoreGraphics APIs on the main thread.
/// On Windows, delegates to the windows_screenshot module which uses GDI BitBlt.
#[cfg(target_os = "macos")]
#[cfg_attr(coverage_nightly, coverage(off))]
#[cfg_attr(not(coverage), tauri::command)]
