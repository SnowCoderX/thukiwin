  ...
  return (
    <div className="p-3 space-y-3 bg-surface-base">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onClose}
          className="text-text-secondary hover:text-text-primary text-xs w-6 h-6 flex items-center justify-center rounded hover:bg-white/8 transition-colors"
        >
          ✕
        </button>
      </div>
        ...
        {profiles.map((profile) => (
          ...
          >
            ...
            {profile.systemPrompt && (
              <p className="text-[11px] text-text-secondary line-clamp-2 mb-0.5">
                {profile.systemPrompt}
              </p>
            )}
            {profile.userContext && (
      ...
      {(isCreating || editingId) && (
        <div className="rounded-lg border border-surface-border bg-surface-elevated p-3 space-y-3">
          ...
          />
          <textarea
            placeholder="System prompt — what the AI should do"
          ...
          />
          <textarea
            placeholder="User context — who you are, your goals, preferences"
